...
// --- ÖĞRENCİ PANELİ (SOL MENÜLÜ YAPI) ---
app.get('/panel/:kullaniciAdi', async (req, res) => {
    ...
    let icerik = "";
    if (mod === 'profil') {
        icerik = `<div style="background:white; padding:30px; border-radius:15px; box-shadow:0 5px 15px rgba(0,0,0,0.05);"><h2>Profil Bilgileri</h2><p><b>Kullanıcı Adı:</b> ${k.kullaniciAdi}</p><p><b>Okul:</b> ${k.okul}</p><p><b>İl/İlçe:</b> ${k.il} / ${k.ilce}</p><p><b>Toplam Puan:</b> ${k.puan}</p><p><b>Çözülen Soru:</b> ${k.soruIndex}</p></div>`;
    } else {
...
app.post('/cevap', async (req, res) => {
    try {
        ...
        if (s && k) {
            ...
            if (dogruMu) {
                // Dinamik Puanlama Mantığı (Z-Skoru bazlı)
...
app.get('/admin', async (req, res) => {
    ...
    if (user === (process.env.ADMIN_USER || 'admin') && pass === (process.env.ADMIN_PASSWORD || '1234')) {
        ...
        res.send(`<div style="max-width:900px; margin:30px auto; font-family:sans-serif; padding:20px; background:#fdfdfd; border-radius:15px; box-shadow:0 10px 30px rgba(0,0,0,0.1);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;"><h2>🛠️ Admin Paneli</h2></div><div style="background:#fff; padding:25px; border:1px solid #e0e0e0; border-radius:12px; margin-bottom:30px;"><form action="${editSoru ? '/soru-guncelle' : '/soru-ekle'}" method="POST">${editSoru ? `<input type="hidden" name="id" value="${editSoru._id}">` : ''}Ders: <select name="ders">${dersler.map(d => `<option value="${d}" ${editSoru && editSoru.ders === d ? 'selected' : ''}>${d}</option>`).join('')}</select><br><br><textarea name="soruMetni" placeholder="Soru Metni" style="width:98%; height:80px; padding:10px; margin-bottom:10px; border:1px solid #ddd;" required>${editSoru ? editSoru.soruMetni : ''}</textarea><div style="background:#f8f9fa; padding:15px; border-radius:10px; margin-bottom:20px;"><p>Şıklar:</p>${[0,1,2,3].map(i => `<div style="margin-bottom:8px; display:flex; align-items:center; gap:5px;"><b>${String.fromCharCode(65+i)}:</b> <input name="metin${i}" value="${editSoru && editSoru.secenekler[i] ? editSoru.secenekler[i].metin : ''}" style="width:40%;"> <input type="radio" name="dogruCevap" value="${i}" ${editSoru && editSoru.dogruCevapIndex === i ? 'checked' : ''} required></div>`).join('')}</div><button style="background:#34a853; color:white; padding:12px 30px; border:none; border-radius:8px; cursor:pointer;">KAYDET</button></form><hr style="margin:30px 0;"><div style="display:grid; gap:10px;">${tumSorular.map((s, i) => `<div style="padding:15px; background:#fff; border:1px solid #eee; border-radius:8px; display:flex; justify-content:space-between; align-items:center;"><span><b>${i+1}.</b> ${s.soruMetni.substring(0,50)}...</span><div><a href="/admin?duzenle=${s._id}">DÜZENLE</a> | <form action="/soru-sil" method="POST" style="display:inline;"><input type="hidden" name="id" value="${s._id}"><button style="color:red; background:none; border:none; cursor:pointer;">SİL</button></form></div></div>`).join('')}</div></div></div>`);
    } else { res.status(401).send('Yetkisiz!'); }
});

app.post('/soru-ekle', async (req, res) => {
    await new Soru({ ders: req.body.ders, soruMetni: req.body.soruMetni, secenekler: [{ metin: req.body.metin0 }, { metin: req.body.metin1 }, { metin: req.body.metin2 }, { metin: req.body.metin3 }], dogruCevapIndex: parseInt(req.body.dogruCevap) }).save();
    res.redirect('/admin');
});

app.post('/soru-guncelle', async (req, res) => {
    await Soru.findByIdAndUpdate(req.body.id, { ders: req.body.ders, soruMetni: req.body.soruMetni, secenekler: [{ metin: req.body.metin0 }, { metin: req.body.metin1 }, { metin: req.body.metin2 }, { metin: req.body.metin3 }], dogruCevapIndex: parseInt(req.body.dogruCevap) });
    res.redirect('/admin');
});
