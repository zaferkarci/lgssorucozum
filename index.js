...
app.post('/cevap', async (req, res) => {
    try {
        ...
        if (s && k) {
            // SADECE secilenIndex GELMİŞSE (TIKLAMA VARSA) İSTATİSTİK GÜNCELLE
            if (secilenIndex !== undefined && secilenIndex !== "") {
                ...
                if (dogruMu) {
                    ...
                    if (dersSorulari.length > 1) {
                        ...
                        if (zorlukSkoru < -1.2) kazanilanPuan = 5;
                        else if (zorlukSkoru < -0.5) kazanilanPuan = 8;
                        else if (zorlukSkoru < 0.5) kazanilanPuan = 12;
                        else if (zorlukSkoru < 1.2) kazanilanPuan = 16;
                        else kazanilanPuan = 22;
                    }
                    k.puan += kazanilanPuan;
                }
            }
            // HER DURUMDA (TIKLAMA OLSA DA OLMASA DA) SORU İNDEKSİNİ ARTIR
