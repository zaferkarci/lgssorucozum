...
app.get('/', (req, res) => {
    res.send(`<div style="text-align:center; padding-top:50px; font-family:sans-serif;"><h2>LGS Giriş</h2><form action="/giris" method="POST" style="max-width:300px; margin:auto;"><input name="kullaniciAdi" placeholder="Kullanıcı Adı" required style="width:90%; padding:10px;"><br><br><input type="password" name="sifre" placeholder="Şifre" required style="width:90%; padding:10px;"><br><br><button style="width:95%; padding:10px; background:green; color:white; border:none; cursor:pointer;">GİRİŞ YAP</button></form><p><a href="/kayit">Kayıt Ol</a></p></div>`);
});
...
app.get('/kayit', (req, res) => {
    ...
    <div style="text-align:center; padding-top:30px; font-family:sans-serif;">
        <h2>Yeni Kayıt</h2>
        <form action="/kayit-yap" method="POST" style="max-width:400px; margin:auto; border:1px solid #ddd; padding:20px;">
            <input name="kullaniciAdi" placeholder="Kullanıcı Adı" required style="width:90%; padding:10px;"><br><br>
            <input type="password" name="sifre" placeholder="Şifre" required style="width:90%; padding:10px;"><br><br>
            <input type="password" name="sifreTekrar" placeholder="Şifre Tekrar" required style="width:90%; padding:10px;"><br><br>
            <select name="il" id="ilSelect" onchange="ilDegisti()" required style="width:95%; padding:10px;">
                <option value="">İl Seçiniz...</option><option value="Aydın">Aydın</option><option value="İzmir">İzmir</option>
            </select><br><br>
            <select name="ilce" id="ilceSelect" onchange="ilceDegisti()" required style="width:95%; padding:10px;"><option value="">İlçe Seçiniz</option></select><br><br>
            <select name="okul" id="okulSelect" required style="width:95%; padding:10px;"><option value="">Okul Seçiniz</option></select><br><br>
            <button style="width:95%; padding:15px; background:blue; color:white;">KAYDI TAMAMLA</button>
        </form>
    </div>
    <script>
        const veriler = {
        ...
        function ilDegisti() {
            ...
            if(il && veriler[il]) { Object.keys(veriler[il]).forEach(ilce => { ilceSelect.innerHTML += '<option value="'+ilce+'">'+ilce+'</option>'; }); }
            ilceDegisti();
        }
        function ilceDegisti() {
            ...
            if(il && ilce && veriler[il][ilce]) { veriler[il][ilce].forEach(okul => { okulSelect.innerHTML += '<option value="'+okul+'">'+okul+'</option>'; }); }
        }
...
app.post('/giris', async (req, res) => {
    ...
    if (!k) return res.send("<script>alert('Hatalı!'); window.history.back();</script>");
...
app.get('/soru/:kullaniciAdi', async (req, res) => {
    ...
    <div style="max-width:600px; margin:auto; font-family:sans-serif;">
        <p>Puan: ${k.puan} | Süre: <span id="timer">00:00</span></p>
        <h3>${soru.soruMetni}</h3>
        ${soru.secenekler.map((s,i)=>`
            <form method="POST" action="/cevap" id="f${i}">
                ...
                <button type="button" onclick="document.getElementById('gs${i}').value=saniye; document.getElementById('f${i}').submit();" style="width:100%; padding:10px; margin:5px;">${s.metin}</button>
            </form>`).join('')}
    </div>
    <script>
        let saniye = 0;
...
// ADMIN PANELİ (Hata Düzeltildi: [0,1,2,3] eklendi)
app.get('/admin', async (req, res) => {
    const tumSorular = await Soru.find();
    res.send(`<h2>🛠️ Admin Paneli</h2><form action="/soru-ekle" method="POST"><textarea name="soruMetni" required></textarea><br>${[0,1,2,3].map(i => `Şık ${i+1}: <input name="metin${i}" required> <input type="radio" name="dogruCevap" value="${i}" required><br>`).join('')}<button>EKLE</button></form><hr>${tumSorular.map(s => `<div>${s.soruMetni.substring(0,30)}</div>`).join('')}`);
});
