...
app.get('/kayit', (req, res) => {
    const iller = ["Adana","Adıyaman","Afyonkarahisar","Ağrı","Aksaray","Amasya","Ankara","Antalya","Ardahan","Artvin","Aydın","Balıkesir","Bartın","Batman","Bayburt","Bilecik","Bingöl","Bitlis","Bolu","Burdur","Bursa","Çanakkale","Çankırı","Çorum","Denizli","Diyarbakır","Düzce","Edirne","Elazığ","Erzincan","Erzurum","Eskişehir","Gaziantep","Giresun","Gümüşhane","Hakkari","Hatay","Iğdır","Isparta","İstanbul","İzmir","Kahramanmaraş","Karabük","Karaman","Kars","Kastamonu","Kayseri","Kilis","Kırikkale","Kırklareli","Kırşehir","Kocaeli","Konya","Kütahya","Malatya","Manisa","Mardin","Mersin","Muğla","Muş","Nevşehir","Niğde","Ordu","Osmaniye","Rize","Sakarya","Samsun","Şanlıurfa","Siirt","Sinop","Sivas","Şırnak","Tekirdağ","Tokat","Trabzon","Tunceli","Uşak","Van","Yalova","Yozgat","Zonguldak"];
...
app.get('/panel/:kullaniciAdi', async (req, res) => {
    ...
    if (mod === 'profil') {
        ...
        if (!sorular.length) {
            ...
            const dersSorulari = await Soru.find({ ders: soru.ders, cozulmeSayisi: { $gt: 0 } }).select('dogruSayisi cozulmeSayisi ortalamaSure').lean();
...
app.post('/soru-ekle-json', async (req, res) => {
    try {
        const data = JSON.parse(req.body.jsonSoru);
        await new Soru(data).save();
        res.redirect('/admin?mod=soruListesi');
    } catch (err) { res.status(500).send("JSON Hatası: " + err.message); }
});
