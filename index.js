...
// Soru Çözme ve Dinamik Skor (sadece gerekli kısımlar)
app.post('/soru/cevapla', async (req, res) => {
    const { soruId, ogrenciDogru, ogrenciSure } = req.body;
    const s = await Soru.findById(soruId);
    if(!s) return res.status(404).send("Soru bulunamadı");

    // 1️⃣ Veri Güncelle
    s.cozulmeSayisi += 1;
    if(ogrenciDogru) s.dogruSayisi += 1;
    s.ortalamaSure = ((s.ortalamaSure * (s.cozulmeSayisi - 1)) + ogrenciSure) / s.cozulmeSayisi;

    // 2️⃣ Standart sapma ve z-score hesaplama
    const dersSorulari = await Soru.find();
    const basariOranlari = dersSorulari.map(q => q.dogruSayisi / (q.cozulmeSayisi || 1));
    const ortalamaBasari = basariOranlari.reduce((a,b)=>a+b,0)/basariOranlari.length;
    const sBasari = Math.sqrt(basariOranlari.reduce((a,b)=>a+Math.pow(b-ortalamaBasari,2),0)/basariOranlari.length) || 1;

    const sureler = dersSorulari.map(q => q.ortalamaSure);
    const ortalamaSure = sureler.reduce((a,b)=>a+b,0)/sureler.length;
    const sSure = Math.sqrt(sureler.reduce((a,b)=>a+Math.pow(b-ortalamaSure,2),0)/sureler.length) || 1;

    const basariOrani = s.dogruSayisi / s.cozulmeSayisi;
    const zB = (basariOrani - ortalamaBasari)/sBasari;
    const zS = (s.ortalamaSure - ortalamaSure)/sSure;

    // 3️⃣ Zorluk ve Dinamik Puan
    let zorluk = (zS * 0.5) - (zB * 0.5);
    let Z_katsayi = 3;
