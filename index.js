// --- LGS HAZIRLIK PLATFORMU - VERSİYON 1.9 (Sınırsız Karmaşıklık Skoru) ---
...
const Soru = mongoose.model('Soru', new mongoose.Schema({
    ...
    guncelZorluk: { type: Number, default: 10 } // Bu alan artık ucu açık "Soru Skoru"dur.
}));

// --- GÜNLÜK SINIRSIZ SKOR HESAPLAMA (05:00 AM) ---
cron.schedule('0 5 * * *', async () => {
    try {
        ...
        for (const ders of dersler) {
            ...
            if (ds.length > 1) {
                ...
                for (const s of ds) {
                    ...
                    // V1.9 Formülü: Negatif zB (düşük başarı) ve Pozitif zS (yüksek süre) skoru artırır.
        ...
        console.log("✅ Günlük Sınırsız Soru Skorları Güncellendi.");
...
app.get('/panel/:kullaniciAdi', async (req, res) => {
    ...
    let icerik = "";
    if (mod === 'profil') {
        // V1.9: Ortalama Puan Hesabı (Tüm karakterler korundu)
...
app.post('/cevap', async (req, res) => {
    try {
        ...
        if (s && k) {
            ...
            if (dogruMu) {
                // V1.9: Sınırsız Soru Skoru üzerinden Logaritmik Puanlama
                ...
                // V1.9: Puan = (Karmaşıklık * Logaritmik Verim * Gelişim Katsayısı)
...
app.get('/admin', async (req, res) => {
    ...
    if (user === (process.env.ADMIN_USER || 'admin') && pass === (process.env.ADMIN_PASSWORD || '1234')) {
        ...
        if (mod === 'soruEkle') {
            ...
            icerik = `<div style="background:white; padding:25px; border:1px solid #e0e0e0; border-radius:12px;"><h3>Tüm Sorular</h3><div style="display:grid; gap:10px;">${tumSorular.map((s, i) => `<div style="padding:15px; background:#fff; border:1px solid #eee; border-radius:8px; display:flex; justify-content:space-between; align-items:center;"><span><b>${i+1}.</b> [Skor: ${s.guncelZorluk.toFixed(2)}] [${s.ders}] ${s.soruMetni.substring(0,40)}...</span><div><a href="/admin?duzenle=${s._id}&mod=soruEkle" style="color:#1a73e8; font-weight:bold; text-decoration:none; margin-right:10px;">DÜZENLE</a><form action="/soru-sil" method="POST" style="display:inline;"><input type="hidden" name="id" value="${s._id}"><button style="color:red; background:none; border:none; cursor:pointer; font-weight:bold;">SİL</button></form></div></div>`).join('')}</div></div>`;
        }
