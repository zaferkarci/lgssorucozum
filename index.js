// --- LGS HAZIRLIK PLATFORMU - VERSİYON 2.3 (TAM SÜRÜM) ---
...
app.get('/panel/:kullaniciAdi', async (req, res) => {
    ...
    if (mod === 'profil') {
        ...
        if (!sorular.length) {
            ...
            icerik = `<div style="text-align:center; margin-top:100px;"><h1 style="color:#1a73e8;">Hazır mısın?</h1><p>Sıradaki soruyu çözmek için butona tıkla.</p><a href="/panel/${encodeURIComponent(k.kullaniciAdi)}?mod=soru&basla=true" style="display:inline-block; padding:15px 40px; background:#34a853; color:white; text-decoration:none; border-radius:30px; font-weight:bold; font-size:18px;">SORU ÇÖZMEYE BAŞLA</a></div>`;
        } else {
...
app.listen(PORT, () => console.log(`🚀 Sunucu ${PORT} portunda v2.3
